...
%RUNUAT_APP_FULL_PATH% BuildCookRun -targetplatform=Win64 -NoP4 -UTF8Output -cook -build -stage -pak -package -project=%MYPROJECT_CONFIG_FULL_PATH% -map=%BIN_APP_START_MAP% -cookdir=%BIN_PLATFORM_OUT_FULL_PATH% -archive -archivedirectory=%BIN_PLATFORM_OUT_FULL_PATH% -ue4exe=UE4Editor-Win64-Debug.exe -clientconfig=Development -NoCompileEditor -deploy
