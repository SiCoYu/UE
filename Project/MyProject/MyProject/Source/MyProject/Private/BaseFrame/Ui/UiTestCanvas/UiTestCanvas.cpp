...
void UUiTestCanvas::loadCanvas()
{
	...
	UButton* pButton = (UButton*)(mWinRender->mUiRoot->GetWidgetFromName("Button_32"));
...
void UUiTestCanvas::loadNamedSlot()
{
	...
	mWinRender->mUiRoot->AddToViewport();	// 一定要先 AddToViewport ，然后再获取控件，否则 GetWidgetFromName 的时候会报错
	// 目前已经有点 NamedSlot 是 "NamedSlot_28"
	mWinRender->mUiRoot->GetSlotNames(m_namedSlotArr);
	m_namedSlot = (UNamedSlot*)(mWinRender->mUiRoot->GetWidgetFromName("NamedSlot_28"));
...
void UUiTestCanvas::loadOverlay()
{
	...
	m_overlay = (UOverlay*)(mWinRender->mUiRoot->GetWidgetFromName("Overlay_17"));
	UWidget* widget = mWinRender->mUiRoot->GetWidgetFromName("Button_32");
}

void UUiTestCanvas::unloadCanvas()
{
	mWinRender->mUiRoot->RemoveFromViewport();
	mWinRender->mUiRoot = nullptr;
}
