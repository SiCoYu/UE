...
function M:onPrefabLoaded(dispObj)
    ...
    for i=1, CtxExt.mPlayerData.mGameData.ranklistCount do
    ...
    for i=1, CtxExt.mPlayerData.mGameData.ranklistCount do
        if(CtxExt.mPlayerData.mGameData.rankinfolist[i].m_rank == CtxExt.mPlayerData.mGameData.myRank) then
            ...
            if CtxExt.mPlayerData.mGameData.myRank > 3 then
                ...
                elseif CtxExt.mPlayerData.mGameData.myRank == 2 then
            ...
            local avatarindex = CtxExt.mPlayerData.mGameData.rankinfolist[i].m_avatarindex;
            ...
            myName.text = CtxExt.mPlayerData.mGameData.rankinfolist[i].m_name;

            --[[
            --本轮质量
            local myMass = GlobalNS.UtilEngineWrap.getComByPath(self.mMyRankArea, "Mass", "Text");
            local radius = GlobalNS.UtilMath.getRadiusByMass(CtxExt.mPlayerData.mGameData.rankinfolist[i].m_radius); --服务器传过来的是质量
            ...
            mySwallowNum.text = CtxExt.mPlayerData.mGameData.rankinfolist[i].m_swallownum;
...
function M:SetTopXRankInfo()
    for i=1, CtxExt.mPlayerData.mGameData.ranklistCount do
        ...
        Name.text = CtxExt.mPlayerData.mGameData.rankinfolist[i].m_name;
        if CtxExt.mPlayerData.mGameData.rankinfolist[i].m_rank == CtxExt.mPlayerData.mGameData.myRank then
            Name.text = "<color=#32c832ff>"..CtxExt.mPlayerData.mGameData.rankinfolist[i].m_name.."</color>";
        end
        ...
        SwallowNum.text = CtxExt.mPlayerData.mGameData.rankinfolist[i].m_swallownum;
        if CtxExt.mPlayerData.mGameData.rankinfolist[i].m_rank == CtxExt.mPlayerData.mGameData.myRank then
            SwallowNum.text = "<color=#32c832ff>"..CtxExt.mPlayerData.mGameData.rankinfolist[i].m_swallownum.."</color>";
        end
    end
end

function M:updateUIData()
    if #self.listitems == CtxExt.mPlayerData.mGameData.ranklistCount then        
        self:SetMyRankInfo();
