...
class LoadParam : IRecycle
{
public:
	ResPackType m_resPackType;           // 加载资源的类型
    ResLoadType m_resLoadType;           // 资源加载类型

    string m_path = "";                  // 资源路径，传递进来都是完成的路径，都是相对 Prefabs\Resources 开始的，例如 Table\CardBase_client.txt，然后内部解析后
    string m_subPath = "";               // 子目录，可能一个包中有多个资源
    string m_pathNoExt = "";             // 这个数据变成了从 Resources 目录开始，没有扩展名字，打包的包名字在包加载的时候判断
	string m_version = "";               // 加载的资源的版本号

protected:
	string m_prefabName = "";         // 预设的名字，就是文件名字，没有路径和扩展名字
    string m_extName = "prefab";      // 加载的资源的扩展名字

    string m_lvlName = "";               // 关卡名字
    public Action<IDispatchObject> m_loadEventHandle;    // 加载事件回调函数

    bool m_resNeedCoroutine = true;      // 资源是否需要协同程序
