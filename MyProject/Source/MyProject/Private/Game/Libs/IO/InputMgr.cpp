...
Rigidbody2D InputMgr::FindRootRigidbody2D(Transform trans)
{
	while (trans != null)
	{
		//if (trans.GetComponent<UIPanel>() != null) return null;
...
bool InputMgr::Raycast(Vector3 inPos)
{
	for (int i = 0; i < list.size; ++i)
	{
		UICamera cam = list.buffer[i];
...
bool InputMgr::IsVisible(Vector3 worldPoint, GameObject go)
{
...
bool InputMgr::IsVisible(ref DepthEntry de)
{
...
bool InputMgr::IsHighlighted(GameObject go)
{
...
int InputMgr::GetDirection(KeyCode up, KeyCode down)
{
...
int InputMgr::GetDirection(KeyCode up0, KeyCode up1, KeyCode down0, KeyCode down1)
{
...
int InputMgr::GetDirection(string axis)
{
...
void InputMgr::Notify(GameObject go, string funcName, object obj)
{
