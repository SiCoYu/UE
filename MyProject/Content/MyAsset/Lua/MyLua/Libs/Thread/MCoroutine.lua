...
function M:ctor()
    self.mFunctor = nil;
    self.mHandle = 0;
    self.mIsError = false;       -- 协程最后是否执行发生错误
end
...
function M:createFixParam(pThis, func, param)
    ...
    self.mFunctor = GlobalNS.new(GlobalNS.CallFuncObjectFixParam);
    self.mFunctor:setPThisAndHandle(pThis, func, param);
    self.mHandle = coroutine.create(runInternal);
end
...
function M:createVarParam(pThis, func, ...)
    ...
    self.mFunctor = GlobalNS.new(GlobalNS.CallFuncObjectVarParam);
...
function M:resume()
    ...
    local status, value = coroutine.resume(self.mHandle)
    self.mIsError = not status;
...
function M:status()
    return coroutine.status(self.mHandle);
end

function M:getStatus()
    local status = coroutine.status(self.mHandle);
...
function M:error(status, value)
    if not status then
        ...
        --value = debug.traceback(self.mHandle, "", 2);
        value = debug.traceback(self.mHandle);                 -- 这个比较准
        --value = debug.traceback(self.mHandle, value);        -- 直接获取当前堆栈，并且添加一些信息在前面
        --value = debug.traceback(self.mHandle, value, 2);     -- 获取堆栈上第二层，将当前函数层去掉              
        error(value)              
    end
end
